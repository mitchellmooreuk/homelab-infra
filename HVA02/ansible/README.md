...
This directory contains Ansible automation used to secure, harden, and configure network infrastructure on the **HVA02** Proxmox VE hypervisor node.
...
```bash
[proxmox_nodes]
HVA02 ansible_host=ip_address_of_HVA02 ansible_user=root
```
...
```bash
ssh -o PubkeyAuthentication=no root@10.25.25.249
``` 
